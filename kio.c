...
int main(void) {
	int menu, idx = 0; //메뉴
	int sum = 0; // 총 금액
	product prd[10] = { 0 }; // 품목, 최대 10종류까지 가능
	// 배열 초기화 때 [크기] 쓰지않으면 자동으로 크기 설정됨. 이때 주의해야 함. 자동으로 설정된 배열의 크기가 아닌 더 큰 크기의 배열을 사용하고 싶다면 사용자가 크기 설정을 해줘야 함(Run-Time check Failure #2 -stack around the variable'변수' was corrupted)
